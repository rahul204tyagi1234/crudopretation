// function ConverAge(age){
//     return age*365;
// }
// console.log(ConverAge(20));

document.querySelector("#submit").addEventListener("click",function
(){
    const add = function (){
        let dob=document.querySelector("#dob").value,
        day =new Date(dob),
        time =day.getTime(),
        today= new Date(),
        timenow=today.getTime(),
        value=timenow-time,
        second = Math.floor(value/1000),
        minut = Math.floor(value/(1000*60)),
        hour = Math.floor(value/(1000*60*60)),
        days = Math.floor(value/(1000*60*60*24)),
        week = Math.floor(value/(1000*60*60*24*7)),
        month = Math.floor(value/(1000*60*60*24*30.4375)),
        year = Math.floor(value/(1000*60*60*24*365.25));
        
        let insert =function(x,y){
            document.querySelector(x).innerHTML =y;
        }
        insert("#years",year +"-Years");
        insert("#Months",month +"-Months");
        insert("#Weaks",week +"-Weaks");
        insert("#Days",days+"-Days");
        insert("#hours",hour +"-hours");
        insert("#minuts",minut +"-minuts");
        insert("#Seconds",second +"-Seconds");
         console.log("interval call.. ")

    }
    setInterval(add,1000);
})